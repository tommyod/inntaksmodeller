...
get_ipython().run_line_magic('timeit', 'evaluate(x, grades, priorities)')
...
# Sorter elevene fra høyest til lavest karaktersnitt
elever = sortert_etter_karakter(elever)

# Gå gjennom alle elever
for elev in elever:
    # Gå gjennom ønsker i prioritert rekkefølge
    for skole in elev.onsker:
        # Dersom skolen har kapasitet, plasser eleven
        if skole.kapasitet > 0:
            skole.plasser(elev)
            break # Gå videre til neste elev
    
# Plasser elevene som ikke fikk ønsker tilfeldig
for elev in elever:
    if not elev.er_plassert():
        tilfeldig_skole = skoler.tilfeldig()
        skole.plasser(elev)
...
# Hver elev har 3 prioriteter, gå gjennom fra høyest til lavest
for prioritet in [1, 2, 3]:
    
    # Gå gjennom alle skolene, rekkefølgen er uviktig
    for skole in skoler:
        # Hent alle elever med riktig prioritet
        elever = skole.elever_med_prioritet(prioritet)
        # Sorter elevene fra høyest til lavest karaktersnitt
        elever = sortert_etter_karakter(elever)
        
        # Gå gjennom elevene sortert etter karaktersnitt
        for elev in elever:
            # Dersom skolen har kapasitet or eleven ikke er plassert
            if skole.kapasitet > 0 and not elev.er_plassert():
                skole.plasser(elev)
        
# Plasser elevene som ikke fikk ønsker oppfylt tilfeldig
for elev in elever:
    if not elev.er_plassert():
        tilfeldig_skole = skoler.tilfeldig()
        skole.plasser(elev)
...
non_dominated_solutions = [x] # Initial solution (e.g. random)

for search_iteration in range(100): # Main loop
    found_solutions = []
    for solution in non_dominated_solutions:
        for search_direction in search_directions:
            # Run simulated annealing and add every solution found
    ...
    non_dominated_solutions = update(non_dominated_solutions)
